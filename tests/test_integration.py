...
from forth_ai_underwriting.prompts.prompt_manager import get_prompt_manager
...
def mock_contact_data() -> dict[str, Any]:
    ...
    return {
        ...
        "custom_fields": {
            "hardship_description": "Lost my job due to company downsizing and medical expenses."
        },
...
def mock_contract_data() -> dict[str, Any]:
    ...
    return {
        ...
        "legal_plan": {"ssn": "123-45-6789", "signed": True},
        "vlp_section": {
            "present": True,
            "signed": True,
            "ssn": "123-45-6789",
...
class TestServiceIntegration:
    ...
    async def test_prompt_manager_integration(self) -> None:
        ...
        contract_prompt = prompt_manager.render_prompt(
            "contract_extraction_v1", document_text="Sample contract text"
        )
        ...
        hardship_prompt = prompt_manager.render_prompt(
            "hardship_assessment_v1", hardship_description="Lost my job"
        )
...
class TestValidationPipeline:
    ...
    async def test_complete_validation_pipeline(
        self, mock_contact_data: dict[str, Any], mock_contract_data: dict[str, Any]
    ) -> None:
        """Test the complete validation pipeline from end to end."""

        # Mock the Forth API client
        with patch(
            "forth_ai_underwriting.services.validation.ForthAPIClient"
        ) as mock_client:
            ...
            with patch(
                "forth_ai_underwriting.services.validation.get_gemini_service"
            ) as mock_gemini:
                ...
                for expected_type in expected_types:
                    assert any(
                        expected_type in vtype for vtype in validation_types
                    ), f"Missing validation: {expected_type}"

                # Check that hardship validation passed
                hardship_result = next(
                    (r for r in results if "Hardship" in r.title), None
                )
    ...
    @pytest.mark.asyncio
    async def test_validation_with_failures(
        self, mock_contact_data: dict[str, Any], mock_contract_data: dict[str, Any]
    ) -> None:
        ...
        failing_contact_data["budget_analysis"] = {
            "income": 3000,
            "expenses": 4000,
        }  # Negative surplus
        ...
        with patch(
            ...
            with patch(
                ...
                ip_result = next(
                    (r for r in results if "IP Addresses" in r.title), None
                )
                assert ip_result is not None
                assert ip_result.result == "No Pass"

                budget_result = next(
                    (r for r in results if "Budget Analysis" in r.title), None
                )
...
class TestErrorHandling:
    ...
    async def test_ai_service_error_handling(self) -> None:
        ...
        with patch(
            "forth_ai_underwriting.services.gemini_service.get_gemini_service"
        ) as mock_get_service:
            mock_service = AsyncMock()
            mock_service.parse_contract_document.side_effect = ValueError(
    ...
    @pytest.mark.asyncio
    async def test_validation_service_error_recovery(
        self, mock_contact_data: dict[str, Any]
    ) -> None:
        ...
        with patch(
            ...
            with patch(
                ...
                mock_gemini_instance = AsyncMock()
                mock_gemini_instance.assess_hardship_claim.side_effect = ValueError(
                    "AI service failed"
                )
                mock_gemini.return_value = mock_gemini_instance
    ...
    async def test_concurrent_validations(
        ...
        with patch(
            ...
            with patch(
                ...
                tasks = [
                    validation_service.validate_contact(
                        f"contact_{i}", mock_contract_data
                    )
                    for i in range(5)
                ]
