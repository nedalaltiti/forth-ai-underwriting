...
class DatabaseSettings:
    ...
    def from_environment(cls) -> "DatabaseSettings":
        ...
        if use_aws_secrets:
            try:
                from forth_ai_underwriting.utils.secret_manager import (
                    get_aws_region,
                ...
                if not secret_name:
                    raise ValueError(
                        "AWS_DB_SECRET_NAME is required when USE_AWS_SECRETS=true"
                    )

                logger.info(
                    f"Loading database credentials from AWS Secrets Manager: {secret_name}"
                )
                ...
                logger.error(
                    f"Failed to load database credentials from AWS Secrets Manager: {e}"
                )

                if environment == "production":
                    raise ValueError(
                        "Production environment requires AWS Secrets Manager for database configuration"
                    ) from e

                logger.info(
                    "Falling back to environment variables for database configuration"
                )
        ...
        return cls(
            ...
            sslmode=get_env_var(
                "DB_SSLMODE", "disable" if environment == "development" else "require"
            ),
            pool_size=get_env_var_int("DB_POOL_SIZE", 20),
            max_overflow=get_env_var_int("DB_MAX_OVERFLOW", 30),
            pool_timeout=get_env_var_int("DB_POOL_TIMEOUT", 30),
...
class SecuritySettings:
    ...
    def from_environment(cls) -> "SecuritySettings":
        ...
        if (
            secret_key.lower() in [weak.lower() for weak in weak_secrets]
            and environment == "production"
        ):
            raise ValueError(
                "Secret key appears to be a default/placeholder value in production"
            )
        ...
        return cls(
            ...
            cors_allow_methods=get_env_var_list(
                "CORS_ALLOW_METHODS", ["GET", "POST", "PUT", "DELETE"]
            ),
            cors_allow_headers=get_env_var_list("CORS_ALLOW_HEADERS", ["*"]),
        )
    ...
    def from_environment(cls) -> "GeminiSettings":
        ...
        if use_aws_secrets and secret_name:
            try:
                ...
                region = get_aws_region()
                logger.info(
                    f"Loading Gemini credentials from AWS Secrets Manager: {secret_name}"
                )
                ...
                logger.error(
                    f"Failed to load Gemini credentials from AWS Secrets Manager: {e}"
                )
                logger.info(
                    "Falling back to environment variables for Gemini configuration"
                )
...
class AWSSettings:
    ...
    gemini_secret_name: str | None = None
    ...
    def from_environment(cls) -> "AWSSettings":
        ...
        return cls(
            ...
            region=get_env_var(
                "AWS_REGION", get_env_var("AWS_DEFAULT_REGION", "us-west-1")
            ),
...
class AppSettings:
    ...
    forth_api: ForthAPISettings = field(
        default_factory=ForthAPISettings.from_environment
    )
    llm: LLMSettings = field(default_factory=LLMSettings.from_environment)
    document_processing: DocumentProcessingSettings = field(
        default_factory=DocumentProcessingSettings.from_environment
    )
...
try:
    ...
    logger.info(
        f"✅ Database: PostgreSQL at {settings.database.host}:{settings.database.port}"
    )
