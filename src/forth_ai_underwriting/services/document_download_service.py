...
from ..core.error_handling import handle_expected_errors
from ..core.observability import increment_counter, time_operation, trace_function
from ..infrastructure.external_apis import ForthAPIClient
from ..infrastructure.queue import QueueAdapter, create_queue_adapter
...
class DownloadTask(BaseModel):
    ...
    def from_queue_message(cls, message: dict[str, Any]) -> "DownloadTask":
        """Parse download task from SQS message with multiple format support."""
        try:
            ...
            # Extract task data with fallback patterns
            return cls(
                contact_id=data.get("contact_id")
                or data.get("ContactId")
                or data.get("data", {}).get("contact_id", ""),
                doc_id=data.get("doc_id")
                or data.get("DocId")
                or data.get("data", {}).get("doc_id", ""),
                doc_type=data.get("doc_type")
                or data.get("DocType")
                or data.get("data", {}).get("doc_type", "Contract"),
                doc_name=data.get("doc_name")
                or data.get("DocName")
                or data.get("data", {}).get("doc_name"),
                correlation_id=data.get("correlation_id") or data.get("CorrelationId"),
                retry_count=data.get("retry_count", 0),
            )
        except Exception as e:
            logger.error(f"Failed to parse download task from message: {e}")
...
class DocumentDownloadService:
    ...
    def __init__(
        ...
        logger.info(
            f"📥 Document Download Service configured: {max_concurrent_downloads} concurrent, {poll_interval_seconds}s poll"
        )
    ...
    @trace_function("document_download.run")
    async def run(self):
        ...
        try:
            ...
            while self.running and not self.shutdown_event.is_set():
                try:
                    ...
                    increment_counter("download_service_errors", {"type": "main_loop"})
                    await asyncio.sleep(5)  # Brief pause on error

        finally:
            await self.shutdown()

    @handle_expected_errors("process_queue_batch", "download_service")
    async def _process_queue_batch(self):
        ...
        try:
            ...
            increment_counter("download_service_errors", {"type": "queue_processing"})

    @time_operation(
        "download_task_duration_seconds", {"operation": "process_download_task"}
    )
    @trace_function("document_download.process_task")
    async def _process_download_task(self, task: DownloadTask, receipt_handle: str):
        ...
        async with self.semaphore:  # Control concurrency
            try:
                ...
                increment_counter("documents_downloaded", {"source": "forth_api"})
                ...
                increment_counter("documents_uploaded", {"destination": "s3"})
    ...
    @trace_function("document_download.download_from_forth")
    async def _download_from_forth(self, task: DownloadTask) -> str | None:
        ...
        try:
            ...
            increment_counter("download_errors", {"source": "forth_api"})
            return None

    @trace_function("document_download.upload_to_s3")
    async def _upload_to_s3(
        ...
        try:
            ...
            increment_counter("upload_errors", {"destination": "s3"})
            return None
        finally:
            # Cleanup temp file
            try:
    ...
    async def _handle_task_failure(
        ...
        increment_counter("download_task_failures", {"reason": "processing_error"})
    ...
    async def _verify_service_health(self):
        ...
        if not queue_health.get("accessible", False):
            raise RuntimeError("Queue service is not accessible")
